...
tourRef.where("gameId", "==", gameId).where("endTime", ">", currentdate).get().then(function(querySnapshot) {
    ...
    if(data.indexOf(data) === -1 && data.length){
      $( ".fhusetb" ).append('<div class="gamename"><img id = "gameIMG" class="gameImg" src='+data[0].gameImg+' alt="game card"><span class="game_name_txt">'+data[0].gameName+'</span></div><div class='+data[0].gameName+' </div></div>' );
